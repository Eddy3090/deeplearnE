...
def metrics(original_dir, pred_dir):
    ...
    MIN_MATCH_COUNT = 10 #计算单应矩阵至少需要的匹配点个数
  
    ratio = 0.7 #0.7 ratio of good match
    thresh = 5.0 #5.0 拒绝阈值 RANSAC
  
    CR_seq = [] # cropping ratio sequence
    DV_seq = [] # distortion value sequence
    Pt = np.eye(3)  #用于存储单应矩阵变换的累计
    P_seq = []  #存储不同时刻Pt的list,path_list 
  
    for i in range(length):
        # Load the images in gray scale
        img1 = cv2.imread(os.path.join(original_dir, image_paths[i]), 0)
        img1o = cv2.imread(os.path.join(pred_dir, image_paths[i]), 0)
        ...
        if descriptors1 is not None and descriptors1o is not None:
            matches = bf.knnMatch(descriptors1, descriptors1o, k=2) #k--返回前一个集合中每个点最匹配的前k个点
        else:
            matches = None
        ...
        if i+1 < len(image_paths):
            img2o = cv2.imread(os.path.join(pred_dir, image_paths[i+1]), 0)
...
if __name__ == "__main__":
    input_dir = "/home/xm/project/temp/codeTry/DUTCode/results/Crowd/input_5"
    pred_dir = "/home/xm/project/temp/codeTry/DUTCode/results/Crowd/DUT_5"
    CRCR, DVDV, SS = metrics(input_dir, pred_dir)
    x = 1